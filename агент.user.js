...
(function () {
  ...
  async function запустити(пошук, максСторінок, спочатку) {
    ...
    /**
     * Закладку можна посувати ТІЛЬКИ за ті сторінки, чиї кандидати вже
     * оцінені й записані. Якщо в черзі ще хтось лишився — закладка стоїть
     * перед сторінкою, з якої той хтось прийшов.
     *
     * Інакше виходить так: агент прочитав три сторінки, поставив закладку
     * на третю, пішов до Gemini, впіймав ліміт — і сорок п'ять зібраних
     * людей зникли, бо завтра він почне з четвертої.
     */
    function зберегтиПрогрес(пройдена) {
      var безпечна = черга.length ? черга[0].сторінка - 1 : пройдена;
      GM_setValue(ключПрогресу, Math.max(0, безпечна));
    }

    /** Оцінює порцію; при зриві повертає людей у чергу, щоб закладка їх не перескочила */
    async function оцінитиПорцію(порція) {
      try {
        лічильники.записано += await обробити(пошук, порція, н, помилки);
        return true;
      } catch (e) {
        черга = порція.concat(черга);
        помилки.push(e.message);
        журнал('⛔️ ' + e.message);
        журнал('Ці ' + порція.length + ' кандидатів не втрачені — ' +
               'закладка лишається перед їхньою сторінкою.');
        обірвано = true;
        return false;
      }
    }
    ...
    for (var с = стартова; с <= межа; с++) {
      ...
      картки.forEach(function (к) {
        ...
        к.сторінка = цяСторінка;   // щоб знати, куди відкочувати закладку
        черга.push(к);
        новіТут++;
      });
      ...
      while (черга.length >= н.пачкаКарток * 2) {
        var вдалося = await оцінитиПорцію(черга.splice(0, н.пачкаКарток * 2));
        if (!вдалося) break;
        зберегтиПрогрес(с);
      }
      if (обірвано) break;
    ...
    if (черга.length && !зупинити && !обірвано) {
      var вдалосяРешта = await оцінитиПорцію(черга.splice(0, черга.length));
      if (вдалосяРешта) зберегтиПрогрес(межа);
    }

    журнал('Закладка: сторінка ' + (GM_getValue(ключПрогресу, 0) || 0) +
           '. Наступний запуск почне з ' + ((GM_getValue(ключПрогресу, 0) || 0) + 1) + '-ї.');
