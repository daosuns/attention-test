...
(function () {
  ...
  var ЗА_ЗАМОВЧУВАННЯМ = {
    ...
    // Вмикається, тільки якщо основна модель перевантажена і не відповіла
    // після чотирьох спроб. Порожнє поле вимикає підміну.
    модельЗапасна: 'gemini-3.5-flash',
    відкриватиРезюмеВід: 50,   // з якого чорнового балу відкривати повне резюме
    паузаСторінки: 1200,       // мс між сторінками work.ua
    максСторінокЗаЗапуск: 200, // запобіжник, щоб обхід не крутився вічно
    // Безкоштовний тариф рахує ЗАПИТИ, а не обсяг, тому пачки великі —
    // токенів усе одно витрачається мала частина дозволеного.
    // 5 секунд — це 12 запитів на хвилину: вкладається в 15 RPM у
    // gemini-3.5-flash-lite і не впирається в TPM навіть на важких пачках.
    // Для моделі з тіснішими лімітами число варто підняти, але й без цього
    // агент не зламається: на 429 він перечекає і повторить.
    паузаGeminiСек: 5,
    пачкаКарток: 25,           // скільки карток за один запит до Gemini
    пачкаРезюме: 10            // скільки повних резюме за один запит
  };

  // Для цих полів порожнє значення — свідомий вибір, а не «не заповнено»
  var МОЖНА_ПОРОЖНЄ = ['модельЗапасна'];

  function налаштування() {
    var н = {};
    for (var к in ЗА_ЗАМОВЧУВАННЯМ) {
      var з = GM_getValue(к, ЗА_ЗАМОВЧУВАННЯМ[к]);
      var порожнє = (з === '' || з === undefined || з === null);
      н[к] = (порожнє && МОЖНА_ПОРОЖНЄ.indexOf(к) === -1) ? ЗА_ЗАМОВЧУВАННЯМ[к]
           : (з === undefined || з === null) ? ЗА_ЗАМОВЧУВАННЯМ[к]
           : з;
    }
    return н;
  }
  ...
  // Скільки чекати перед повторною спробою, у секундах
  var ЧЕКАННЯ_ПЕРЕВАНТАЖЕННЯ = [5, 15, 40, 90];  // модель зайнята (500-504)
  var ЧЕКАННЯ_ЛІМІТУ         = [35, 75];         // забагато запитів (429)

  /** Одна спроба звернутись до конкретної моделі */
  async function однаСпроба(модель, ключ, підказка, схема, паузаСек) {
    var зачекати = паузаСек * 1000 - (Date.now() - часОстанньогоGemini);
    if (зачекати > 0) await пауза(зачекати);
    часОстанньогоGemini = Date.now();

    return запит({
      метод: 'POST',
      адреса: 'https://generativelanguage.googleapis.com/v1beta/models/' +
              encodeURIComponent(модель) + ':generateContent?key=' + encodeURIComponent(ключ),
  ...
  function розібратиВідповідь(текстВідповіді) {
    var дані = JSON.parse(текстВідповіді);
    var текст = дані &&
                дані.candidates &&
                дані.candidates[0] &&
  ...
  /**
   * Звертання до Gemini з терпінням.
   *
   * 503 «модель перевантажена» — це не наша помилка, а черга на боці Google,
   * і за хвилину-другу вона зазвичай розсмоктується. Тому не падаємо, а чекаємо.
   * 429 — або забагато запитів за хвилину (мине), або вичерпана денна норма
   * (не мине); розрізняємо це двома спробами.
   */
  async function доGemini(підказка, схема) {
    ...
    var моделі = [н.модель];
    if (н.модельЗапасна && н.модельЗапасна !== н.модель) моделі.push(н.модельЗапасна);

    var паузаСек = Number(н.паузаGeminiСек) || 13;
    var остання = '';

    for (var м = 0; м < моделі.length; м++) {
      var модель = моделі[м];
      if (м > 0) журнал('↻ Переходжу на запасну модель: ' + модель);

      var перевантажень = 0;
      var лімітів = 0;

      while (true) {
        var в = await однаСпроба(модель, н.ключGemini, підказка, схема, паузаСек);

        if (в.код === 200) {
          if (м > 0) журнал('✓ Запасна модель відповіла.');
          return розібратиВідповідь(в.текст);
        }

        // Модель зайнята — це минуще, чекаємо й пробуємо ще
        if (в.код >= 500 && в.код <= 504) {
          if (перевантажень < ЧЕКАННЯ_ПЕРЕВАНТАЖЕННЯ.length) {
            var скільки = ЧЕКАННЯ_ПЕРЕВАНТАЖЕННЯ[перевантажень++];
            журнал('⏳ Модель ' + модель + ' перевантажена. Чекаю ' + скільки + ' с…');
            статус('Модель зайнята, чекаю ' + скільки + ' с (спроба ' + перевантажень + ' з 4)');
            await пауза(скільки * 1000);
            continue;
          }
          остання = 'Модель «' + модель + '» перевантажена на боці Google і не відповіла ' +
                    'після чотирьох спроб';
          break;   // спробуємо запасну, якщо вона є
        }

        // Забагато запитів. Перший раз — можливо, просто за хвилину; далі — денна норма
        if (в.код === 429) {
          if (лімітів < ЧЕКАННЯ_ЛІМІТУ.length) {
            var пауза429 = ЧЕКАННЯ_ЛІМІТУ[лімітів++];
            журнал('⏳ Забагато запитів. Чекаю ' + пауза429 + ' с…');
            статус('Ліміт запитів, чекаю ' + пауза429 + ' с…');
            await пауза(пауза429 * 1000);
            continue;
          }
          throw new Error('ЛІМІТ: безкоштовна норма запитів до Gemini вичерпана. ' +
                          'Прогрес збережено — запусти пізніше, агент продовжить ' +
                          'з тієї ж сторінки. Норма поновлюється о 10:00 за Києвом.');
        }

        // Решта кодів — справжні помилки, повторювати немає сенсу
        throw new Error('Gemini відповів кодом ' + в.код + ': ' + в.текст.slice(0, 200));
      }
    }

    throw new Error(остання + '. Спробуй пізніше або обери в налаштуваннях іншу модель — ' +
                    'старіші зазвичай вільніші.');
  }
  ...
  /**
   * Обхід ведеться не «до сторінки N», а «поки не набереться N НОВИХ людей».
   *
   * Чому не по сторінках: work.ua сортує резюме за свіжістю, і люди додають
   * їх щогодини. За ніч список сповзає вниз на цілу сторінку, тож учорашня
   * закладка «сторінка 3» сьогодні вказує вже на інших людей — а найсвіжіші,
   * що з'явились уночі вгорі, агент не побачив би взагалі.
   *
   * Тому агент завжди починає з першої сторінки. Тих, хто вже в «Історії»,
   * він упізнає і пропускає — це не коштує ні запиту до Gemini, ні відкриття
   * резюме, тільки читання сторінки. Гортає доти, доки не набере потрібну
   * кількість справді нових.
   */
  async function запустити(пошук, скількиНових) {
    var н = налаштування();
    зупинити = false;

    var лічильники = { сторінок: 0, переглянуто: 0, нових: 0, записано: 0, пропущено: 0 };
    var помилки = [];
    ...
    var перша = await сторінка(адресаСторінки(пошук.адреса, 1));
    var всього = скількиСторінок(перша);

    var стеля = Number(н.максСторінокЗаЗапуск) || 0;
    var межа = стеля > 0 ? Math.min(стеля, всього) : всього;

    журнал('Усього сторінок: ' + всього + '. Шукаю ' + скількиНових + ' нових кандидатів.');

    var черга = [];
    var обірвано = false;

    async function оцінитиПорцію(порція) {
      try {
        лічильники.записано += await обробити(пошук, порція, н, помилки);
        return true;
      } catch (e) {
        помилки.push(e.message);
        журнал('⛔️ ' + e.message);
        журнал('Ці ' + порція.length + ' кандидатів не втрачені: їх немає в «Історії», ' +
               'тож наступний запуск підбере їх заново.');
        обірвано = true;
        return false;
      }
    }
    ...
    for (var с = 1; с <= межа; с++) {
      ...
      var doc;
      try {
        doc = (с === 1) ? перша : await сторінка(адресаСторінки(пошук.адреса, с));
      } catch (e) {
      ...
      картки.forEach(function (к) {
        if (лічильники.нових >= скількиНових) return;   // набрали скільки треба
        if (вжеБачили[к.номер]) { лічильники.пропущено++; return; }
        вжеБачили[к.номер] = true;
        черга.push(к);
        лічильники.нових++;
      });

      статус('Сторінка ' + с + ' · нових ' + лічильники.нових + ' з ' + скількиНових +
             ' · записано ' + лічильники.записано);

      // Оцінюємо по ходу, щоб результат зʼявлявся в таблиці, не чекаючи кінця
      while (черга.length >= н.пачкаКарток * 2) {
        if (!await оцінитиПорцію(черга.splice(0, н.пачкаКарток * 2))) break;
      }
      if (обірвано) break;

      if (лічильники.нових >= скількиНових) {
        журнал('Набрано ' + скількиНових + ' нових на сторінці ' + с + '. Далі не гортаю.');
        break;
      }

      if (с === межа && межа < всього) {
        журнал('⚠️ Дійшла до межі в ' + межа + ' сторінок за запуск, а нових набралось ' +
               лічильники.нових + '. Підніми межу в налаштуваннях або звузь фільтр на work.ua.');
      }

      if (с < межа) await пауза(н.паузаСторінки);
    }

    if (черга.length && !зупинити && !обірвано) {
      await оцінитиПорцію(черга.splice(0, черга.length));
    }

    if (лічильники.пропущено) {
      журнал('Пропущено як уже відомих: ' + лічильники.пропущено +
             ' (на них не витрачено жодного запиту до Gemini).');
    }
  ...
  П.innerHTML =
    ...
    '<div id="ар-робота">' +
      ...
      '<input id="ар-скільки" type="number" min="1" value="30" style="width:100%;padding:7px;border-radius:6px;border:1px solid #374151;background:#1f2937;color:#fff;margin:4px 0 6px">' +
      '<div style="font-size:11px;opacity:.6;margin-bottom:10px;line-height:1.4">' +
        'Агент завжди починає з першої сторінки, щоб не проґавити свіжі резюме. ' +
        'Тих, хто вже в «Історії», пропускає без витрат — і гортає стільки ' +
        'сторінок, скільки треба, щоб набрати цю кількість нових.' +
      '</div>' +
  ...
  function намалюватиНалаштування() {
    ...
    html += '<label style="display:block;opacity:.7;font-size:12px;margin-top:8px">Модель Gemini</label>' +
            ...
            '<label style="display:block;opacity:.7;font-size:12px;margin-top:8px">' +
              'Запасна модель (якщо основна перевантажена)</label>' +
            '<select id="ар-модель-запасна" style="width:100%;padding:7px;border-radius:6px;border:1px solid #374151;background:#1f2937;color:#fff">' +
              '<option value=""' + (н.модельЗапасна ? '' : ' selected') + '>— без запасної —</option>' +
              (н.модельЗапасна
                ? '<option value="' + String(н.модельЗапасна).replace(/"/g, '&quot;') +
                  '" selected>' + н.модельЗапасна + '</option>'
                : '') +
            '</select>' +
    ...
    $('ар-моделі').onclick = async function () {
      ...
      try {
        ...
        var запасна = $('ар-модель-запасна').value;
        ...
        $('ар-модель-запасна').innerHTML =
          '<option value=""' + (запасна ? '' : ' selected') + '>— без запасної —</option>' +
          моделі.map(function (м) {
            return '<option value="' + м + '"' + (м === запасна ? ' selected' : '') + '>' + м + '</option>';
          }).join('');
    ...
    $('ар-зберегти').onclick = function () {
      ...
      GM_setValue('модельЗапасна', $('ар-модель-запасна').value);
  ...
  $('ар-старт').onclick = function () {
    ...
    запустити(пошук, Math.max(1, Number($('ар-скільки').value) || 30))
      .catch(function (e) {
        статус('✗ ' + e.message);
        журнал('✗ ' + e.message);
        доТаблиці('журнал', {
