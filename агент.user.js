...
(function () {
  ...
  var ЗА_ЗАМОВЧУВАННЯМ = {
    ...
    паузаGemini: 7000,         // мс між запитами до Gemini (безкоштовний тариф ≈10/хв)
    пачкаКарток: 15,           // скільки карток за один запит до Gemini
    пачкаРезюме: 6             // скільки повних резюме за один запит
  };
  ...
  async function доGemini(підказка, схема) {
    ...
    var чекати = н.паузаGemini - (Date.now() - часОстанньогоGemini);
  ...
  async function обробити(пошук, порція, н, помилки) {
    ...
    журнал('У таблицю записано: ' + в.записано);
    return в.записано;
  }
